...
if (isset($_POST["btnReservar"])) {
    ...
    if (!empty($consulta_disponibilidad)) {
        ...
        $consulta_reserva = "SELECT  id_reserva FROM eventos_reservas where id_cliente = (select c.id_cliente from personas p join clientes c on c.id_persona = p.id_persona where p.usuario = '" . $_SESSION["usuario"] . "') ORDER BY fecha_hora_reserva DESC LIMIT 1";
